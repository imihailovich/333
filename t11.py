...
async def send_final_prompt_topic1(message: types.Message):
    ...
    new_prompt = (
        ...
        "Помоги мне придумать 30 тем для постов, которые помогут мне рассказать моей целевой аудитории о своем проекте, услугах, опыте, ценностях и экспертности.\n"
        "Темы должны быть на разные темы и в разных форматах, чтобы аудитории было интересно, и она вовлекалась. "
        "Например, лайф-пост с фотографиями, полезный подкаст с призывом записаться на бесплатную консультацию, карусель с чек-листом, видеообзор продукта и т.д.\n"
        "К теме поста напиши более подходящий формат (например: сторис, подкаст, чек-лист, карточки, видео, текстовый пост, подборка, инфографика и т.д.)."
    )

    # Теперь делаем запрос к OpenAI и отправляем результат пользователю
    ...
    try:
        ...
        await message.answer(result_text, parse_mode="HTML", reply_markup=workshop_keyboard)
...
# ... остальной код (контент-план, генерация изображения, поддержка, баланс, и т.д.) остается прежним ...
...
async def content_plan_themes(message: types.Message):
    ...
    await message.answer(
        ...
        # ... (оставьте остальной подсказочный текст без изменений) ...
...
async def content_plan_result(message: types.Message):
    ...
    prompt_text = (
        ...
        "Распредели темы в соответствии с выбранным периодом и периодичностью. Для каждого дня/публикации предложи название, формат (пост, сторис, видео, карусель и т.д.), краткое описание и call-to-action.\n"
